...
const {usernameRequirements,birthRequirements, idRequirements} = FormRequirements;
...
const FindPw = () => {
  ...
  const {register, handleSubmit, formState : {errors}} = useForm<findPwProps>();

  const onSubmitHandler: SubmitHandler<findPwProps> = (data) => {
    axios.get(`auth/findPassword`, {
      params : {
        name : data.name,
        birth : data.birth,
        id: data.id
